...
export function ZakatCalculatorComponent() {
  ...
  const calculateZakat = () => {
    ...
    const totalInvestments = Object.values(investments).reduce((sum, investment) => {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-100 to-green-200 flex items-center justify-center p-4">
      <Card className="w-full max-w-2xl">
        <CardHeader>
          <CardTitle className="text-3xl font-bold text-center text-green-800">Islamic Zakat Calculator</CardTitle>
          ...
          <div className="space-y-6">
                ...
                {Object.entries(investments).map(([key, value]) => (
                  <div key={key} className="flex flex-col space-y-1.5">
                    ...
                    <Input
                      ...
                      placeholder={`Enter your ${key.replace(/([A-Z])/g, ' $1').trim()} value`}
                    />
                  </div>
                ))}
              </div>
            </div>
            <Button onClick={calculateZakat} className="w-full bg-green-600 hover:bg-green-700">
              Calculate Zakat
            </Button>
            {zakatAmount !== null && (
              <div className="mt-4 p-4 bg-green-100 rounded-md">
                <h3 className="text-lg font-semibold text-green-800">Your Zakat Amount:</h3>
                <p className="text-2xl font-bold text-green-600">{zakatAmount.toFixed(2)}</p>
              </div>
            )}
            <div className="mt-6 text-sm text-gray-600">
              ...
              <ul className="list-disc list-inside space-y-1">
                ...
                <li>It&apos;s typically calculated as 2.5% of one&apos;s wealth above the nisab threshold.</li>
                <li>The nisab is approximately the value of 85 grams of gold or 595 grams of silver.</li>
                <li>Zakat is paid on various types of wealth, including cash, gold, silver, and investments.</li>
                <li>This calculator provides a basic estimation. For precise calculations, consult with an Islamic scholar.</li>
              </ul>
            </div>
          </div>
        </CardContent>
